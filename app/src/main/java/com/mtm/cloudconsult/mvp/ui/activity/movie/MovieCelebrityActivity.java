 ...
 * 豆瓣电影演员详情页面
 */
public class MovieCelebrityActivity extends BaseActivity<MovieDetailPresenter> implements MovieDetailContract.View {
    ...
    @BindView(R.id.ll_movie_directors)
    LinearLayout llMovieDirectors;
    ...
    public void initData(@Nullable Bundle savedInstanceState) {
        if (getIntent() != null && getIntent().hasExtra(MOVIE_PERSON)) {
            moviePerson = (MoviePerson) getIntent().getSerializableExtra(MOVIE_PERSON);
            llMovieDirectors.setVisibility(View.GONE);
            tvMovieCasts.setVisibility(View.GONE);
        }
        //高斯模糊
        GlideUtils.loadTransformationImage(MovieCelebrityActivity.this, imgItemBg, moviePerson.getAvatars().getLarge(), 95);
        initSlideShapeTheme(moviePerson.getAvatars().getLarge(), imgItemBg);
        setToolBar();
        GlideUtils.loadMovieImage(this, moviePerson.getAvatars().getLarge(), ivOnePhoto);
        tbBaseTitle.setTitle(moviePerson.getName());//标题
        tbBaseTitle.setSubtitle(moviePerson.getName_en());
        initRecycleView();
        if (moviePerson != null) {
    ...
    public void refreshUI(List data) {
        if (ObjectUtils.isNotEmpty(data)) {
            MovieCelebrity listReply = (MovieCelebrity) data.get(0);
            tvOneRatingRate.setText("姓名  " + listReply.getName());
            tvOneDirectors.setText("性别  "+listReply.getGender());
            tvOneCasts.setText("出生日期  "+listReply.getBirthday());
            tvOneGenres.setText("出生地  "+listReply.getBorn_place());
            tvOneDay.setText("星座  "+listReply.getConstellation());
            tvOneCity.setText("职业  "+ StringUtils.formatGenres(listReply.getProfessions()));
    ...
    private void setImgHeaderBg(String imgUrl) {
        if (!TextUtils.isEmpty(imgUrl)) {
            //高斯模糊
            Glide.with(MovieCelebrityActivity.this)
                    .load(imgUrl)
    ...
    public static void startActivity(Activity context, MovieBean positionData, ImageView imageView) {
        Intent intent = new Intent(context, MovieCelebrityActivity.class);
